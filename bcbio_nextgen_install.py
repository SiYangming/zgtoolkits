...
REMOTES = {
    ...
    "anaconda": "https://mirrors.tuna.tsinghua.edu.cn/anaconda/miniconda/Miniconda2-latest-%s-x86_64.sh"
	}
...
def install_anaconda_python(args):
    ...
    if not os.path.exists(anaconda_dir) or not os.path.exists(conda):
        ...
        if not os.path.exists(os.path.basename(url)):
            subprocess.check_call(["wget", "-4", "--progress=dot:mega", "--no-check-certificate", url])
